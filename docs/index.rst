...
The initialization method raises multiple questions regarding the setting of parameter values.
...
(Matrices for grayscale images are represented by :math:`Height \times Width \times 1` matrices, while tensors for RGB images are represented by :math:`Height \times Width \times 3` tensors.)
...
If you pass 0, by default the dataset will be save with the name **dataset_raw.npz**. The dataset produced by this method can be used by the class **BuildDataset** to put together more .npz files and create a dataset like the `mnist`_.
...
* **label**: List of object names to include in the dataset. The constructor method creates a named folder for every image type.
